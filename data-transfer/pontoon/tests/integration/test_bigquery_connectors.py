    ...
    def test_bigquery_destination(self):
        ...
        def connect():
            return create_engine(
                    f"bigquery://{os.environ['BQ_PROJECT_ID']}", 
                    credentials_info=json.loads(open(os.environ['GCP_SERVICE_ACCOUNT_FILE']).read()), 
                    arraysize=1024
            ).connect()
        ...
        def drop():
            with connect() as conn:
                ...
                conn.execute(text("DROP TABLE IF EXISTS target.__temp_pontoon_transfer_test"))
